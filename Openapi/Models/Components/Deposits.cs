...
namespace Openapi.Models.Components
{
    ...
    {
        public static string Value(this Deposits value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static Deposits ToEnum(this string value)
        {
            foreach(var field in typeof(Deposits).GetFields())
            {
                ...
                {
                    return (Deposits)field.GetValue(null);
                }
            }

            throw new Exception($"Unknown value {value} for enum Deposits");
        }
    }
