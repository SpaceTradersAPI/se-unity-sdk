...
namespace Openapi.Models.Components
{
    ...
    public static class ShipTypeJsonExtension
    {
        public static string Value(this ShipTypeJson value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static ShipTypeJson ToEnum(this string value)
        {
            foreach(var field in typeof(ShipTypeJson).GetFields())
            {
                ...
                {
                    return (ShipTypeJson)field.GetValue(null);
                }
            }

            throw new Exception($"Unknown value {value} for enum ShipTypeJson");
        }
    }
