...
namespace Openapi.Models.Components
{
    ...
    public static class WaypointTypeJsonExtension
    {
        public static string Value(this WaypointTypeJson value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static WaypointTypeJson ToEnum(this string value)
        {
            foreach(var field in typeof(WaypointTypeJson).GetFields())
            {
                ...
                {
                    return (WaypointTypeJson)field.GetValue(null);
                }
            }

            throw new Exception($"Unknown value {value} for enum WaypointTypeJson");
        }
    }
